...
# from pytorchvideo.transforms import Normalize as Normalize_video
...
def build_transform(cfg, is_train=True, choices=None):
    ...
    # normalize_video = Normalize_video(mean=cfg.INPUT.PIXEL_MEAN, std=cfg.INPUT.PIXEL_STD)
    
    if is_train:
        return _build_transform_train(cfg, choices, target_size, normalize, normalize)
    else:
        return _build_transform_test(cfg, choices, target_size, normalize, normalize)

def _build_transform_train(cfg, choices, target_size, normalize, normalize_video):
    ...
    if "normalize_video" in choices:
        print(
            f"+ normalization for video (mean={cfg.INPUT.PIXEL_MEAN}, std={cfg.INPUT.PIXEL_STD})"
        )
        tfm_train += [normalize_video]
...
def _build_transform_test(cfg, choices, target_size, normalize, normalize_video):
    ...
    if "normalize_video" in choices:
        print(
            f"+ video normalization (mean={cfg.INPUT.PIXEL_MEAN}, std={cfg.INPUT.PIXEL_STD})"
        )
        tfm_test += [normalize_video]
